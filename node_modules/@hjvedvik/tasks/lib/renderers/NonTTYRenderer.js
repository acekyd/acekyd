...
class NonTTYRenderer extends Renderer {
  render (task) {
    ...
    const { isCurrent, isDone, isFailed, isSkipped, totalTime } = task
    const hasFailed = isFailed && task.exitOnError && !task.__finished
    const hasFinished = ((isDone && totalTime) || isSkipped) && !task.__finished

    if (isCurrent && !task.__started) {
      task.__started = true
