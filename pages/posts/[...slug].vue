    ...
    <article>
      <header class="mb-8 border-b border-border pb-8">
        <div v-if="data?.date || data?.tags?.length" class="mb-3 flex flex-wrap items-center gap-3 text-xs text-ink-faint">
          ...
          <span v-if="minutes" class="inline-flex items-center gap-3">
            <span class="text-ink-faint/50">·</span>
            {{ minutes }} min read
          </span>
      ...
      <details v-if="toc.length >= 3" class="mb-8 rounded-2xl border border-border bg-surface/50 px-5 py-4">
        <summary class="cursor-pointer text-sm font-semibold text-ink">Table of contents</summary>
        <ul class="mt-3 space-y-1.5 text-sm">
          <li v-for="link in toc" :key="link.id">
            <a :href="`#${link.id}`" class="text-ink-faint no-underline transition-colors hover:text-accent-ink">{{ link.text }}</a>
            <ul v-if="link.children?.length" class="ml-4 mt-1.5 space-y-1.5">
              <li v-for="child in link.children" :key="child.id">
                <a :href="`#${child.id}`" class="text-ink-faint no-underline transition-colors hover:text-accent-ink">{{ child.text }}</a>
              </li>
            </ul>
          </li>
        </ul>
...
const minutes = readingMinutes(data.value);
const toc = computed(() => data.value?.body?.toc?.links ?? []);

// Related posts are fetched here (not inside the component) — an async child
// component inside the page's Suspense boundary breaks ContentDoc hydration
// on direct loads/refreshes.
const { data: allPosts } = await useAsyncData(`related-${cleanedPath}`, () =>
  queryContent('/posts')
...
const relatedPosts = computed(() => {
  const mine = new Set(data.value?.tags ?? []);
  const candidates = (allPosts.value ?? [])
    .filter((p) => p._path !== cleanedPath && !p.external_url)
    .map((p) => ({ ...p, score: (p.tags ?? []).filter((t) => mine.has(t)).length }));
...
// Prefer the post's own cover image for social sharing; fall back to the
// branded title card generated by scripts/generate-og-cards.mjs.
const cover = resolveCover(data.value);
const coverAbs = cover ? (cover.startsWith('http') ? cover : 'https://adewaleabati.com' + cover) : null;
const ogImage = coverAbs || ('https://adewaleabati.com/og' + cleanedPath.replace(/^\/posts/, '') + '.png');
